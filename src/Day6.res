...
let arrayUnion = (array) => {
    let head = array[0];
    let queue = Js.Array2.sliceFrom(array, 1);

    Js.Array2.reduce(queue, (result, suArray) => {
        Js.Array2.filter(result, letter => Js.Array2.includes(suArray, letter));
    }, head) -> Js.Array2.length;
}

let result = Js.String.split("\n\n", input) -> Js.Array2.map(value =>  {
    Js.String.split("\n", value) -> Js.Array2.map(Js.String.split("")) -> arrayUnion;
