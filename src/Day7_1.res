...
let quantityRe = Js.Re.fromStringWithFlags("\d ", ~flags="g");
let dotRe = Js.Re.fromStringWithFlags("\.", ~flags="g");
let bagRe = Js.Re.fromStringWithFlags(" bag(s)?", ~flags="g");

let colorDictionary = Js.String2.split(input, "\n") 
...
-> Js.Array2.reduce((acc, (node, children)) => {
    Js.Array2.reduce(children, (acc, child) => {
        switch(Js.Dict.get(acc, child)) {
        | Some(values) => {
            Js.Dict.set(acc, child, Js.Array2.concat(values, [node]));
            acc;
        };
        | None => {
            Js.Dict.set(acc, child, [node]);
            acc;
        };
        }
    }, acc);
...
let rec getAllParentOfNodes = (nodeNames, parents) => {
    let newParents: Js.Array2.t<Js.String2.t> = Js.Array2.reduce(nodeNames, (acc, nodeName) => {
        switch (Js.Dict.get(colorDictionary, nodeName)) {
            | Some([]) => acc; 
            | None => acc;
            | Some(values) => Js.Array2.concat(acc, values);
        };
    ...
    switch(newParents) {
        | [] => parents;
        | _ => getAllParentOfNodes(newParents, Js.Array2.concat(parents, newParents));
    }
}
