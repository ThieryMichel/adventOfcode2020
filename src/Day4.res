let input = `iyr:2010 ecl:gry hgt:181cm
...
let hasValidBirthYear = hasValidProp("byr", byr => {
    switch(Belt.Int.fromString(byr)) {
        | Some(number) => number >= 1920 && number <= 2002;
        | None => false;
    }
});

let hasValidIssueYear = hasValidProp("iyr", (iyr) => {
    switch(Belt.Int.fromString(iyr)) {
        | Some(number) => number >= 2010 && number <= 2020;
        | None => false;
    }
})

let hasValidExpirationYear = hasValidProp("eyr", (eyr) => {
    switch(Belt.Int.fromString(eyr)) {
        | Some(number) => number >= 2020 && number <= 2030;
        | None => false;
    }
})

let parseHeight = (value) => {
    let l = Js.String.length(value);
    let unit = Js.String.substringToEnd(~from = l - 2, value);
    let value = Js.String.substring(~from=0, ~to_ = l - 2, value) |> Belt.Int.fromString;

    (unit, value);
}

let hasValidHeight = hasValidProp("hgt", (hgt) => {
    let (unit, value) = parseHeight(hgt);
    switch(unit, value) {
        | ("in", Some(value)) => value >= 59 && value <= 76; 
        | ("cm", Some(value)) => value >= 150 && value <= 193;
        | _ => false;
    }
});

let hclRe = Js.Re.fromString("^#([0-9]|[a-f]){6}$");
let hasValidHairColor = hasValidProp("hcl", (hcl) => Js.Re.test_(hclRe, hcl));

let validColors = ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"];
let hasValidEyeColor = hasValidProp("ecl", (ecl) => {
    Js.Array.includes(ecl, validColors);
});
