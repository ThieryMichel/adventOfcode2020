...
let childrenByNode = Js.String2.split(input, "\n") 
...
-> Js.Array2.reduce((acc, (node, children)) => {
    switch(Js.Dict.get(acc, node)) {
        | Some(values) => {
            Js.Dict.set(acc, node, Js.Array2.concat(values, children));
            acc;
        };
        | None => {
            Js.Dict.set(acc, node, children);
            acc;
        };
    }
}, Js.Dict.fromArray([]));

let rec countChildrenOfNodes = (name, total) => {
    let values = Js.Dict.get(childrenByNode, name);
    Js.log((name, total, values))
    let r = switch(values) {
        | Some([]) => 0;
        | None => 0;
        | Some(values) => {
            Js.Array2.map(values, ((qty, name)) => {
                qty * countChildrenOfNodes(name, 1);
            }) -> Js.Array2.reduce((acc, qty) => acc + qty, 0);
        }
    }
...
let result = countChildrenOfNodes("shiny gold", 0);
