...
let parsedInput = Js.String2.split(input, "\n")
    ->Belt.Array.map(line => Js.String2.castToArrayLike(line)-> Js.Array.fromMap(v => v))
    ->Belt.Array.map((row) => Belt.Array.map(row, (char) => {
        switch(char) {
        | "." => None;
        | "L" => Some(false);
        | v => Js_exn.raiseError("Could no parse input " ++ v);
        }
    }));

let mapCell = (arrayofArray, fn) => {
    Belt.Array.mapWithIndex(arrayofArray, (y, row) => {
        Belt.Array.mapWithIndex(row, (x, cell) => fn(cell, x, y)); 
    });
}

// let reduceCellWithIndex = (arrayofArray, start, fn) => {
//     Belt.Array.reduceWithIndex(arrayofArray, start, (acc, row, y) => {
//         Belt.Array.reduceWithIndex(row, acc, (acc, cell, x) => fn(acc, cell, x, y)); 
//     });
// }

let countOccupiedCell = (arrayofArray) => {
    Belt.Array.reduce(arrayofArray, 0, (total, row) => {
        Belt.Array.reduce(row, total, (total, cell) => {
            switch (cell) {
            | None => total;
            | Some (false) => total;
            | Some(true) => total + 1;
...
let getCell = (array, x, y) => {
    Belt.Array.get(array, y)
        ->Belt.Option.flatMap(line => Belt.Array.get(line, x));
}

let rec findNeighbor = ((offsetX, offsetY), input, (x, y)) => {
    switch (getCell(input, x + offsetX, y + offsetY)) {
    | None => false;
    | Some(Some(occupied)) => occupied;
    | Some(None) => findNeighbor((offsetX, offsetY), input, (x + offsetX, y + offsetY));
    }
}

let countLineOfSightsOccupiedCell = (array, targetX, targetY) => {
    [
        findNeighbor((-1, 0), array, (targetX, targetY)),
        findNeighbor((1, 0), array, (targetX, targetY)),
        findNeighbor((0, -1), array, (targetX, targetY)),
        findNeighbor((0, 1), array, (targetX, targetY)),
        findNeighbor((-1, -1), array, (targetX, targetY)),
        findNeighbor((1, -1), array, (targetX, targetY)),
        findNeighbor((-1, 1), array, (targetX, targetY)),
        findNeighbor((1, 1), array, (targetX, targetY))
    ]
    -> Js.Array2.filter(v => v) -> Js.Array2.length;
}
...
let changeSeat = (input) => {
    let hasChanged = ref(false);
    let result = mapCell(input, (seat, x, y) => {
        switch (seat) {
            | None => None;
            | Some(occupied) => {
                let neighbors = countLineOfSightsOccupiedCell(input, x, y);
                let neighborsState = if (neighbors <= 0) {
                    Free;
                } else {
                    if (neighbors >= 5) {
                        Crowded
                ...
                switch((occupied, neighborsState)) {
                | (true, Crowded) => {
                    hasChanged := true;
                    Some(false);
                }
    ...
    (result, hasChanged.contents)
}


let rec computeFinalConfiguration = (input) => {
    let (updatedSeat, hasChanged) = changeSeat(input);

    switch (hasChanged) {
    | true => {
        computeFinalConfiguration(updatedSeat);
    }
    | false => input;
    }
}

let finalConfiguration = computeFinalConfiguration(parsedInput);

let result = countOccupiedCell(finalConfiguration);
