...
let rec getInstructionIndexSequence = (instruction, lastIndex, indexes) => {
    ...
    switch(Js.Array2.includes(indexes, nextIndex)) {
        | true => indexes;
        | false => {
            getInstructionIndexSequence(parsedInstructions[nextIndex], nextIndex, Js.Array2.concat(indexes, [nextIndex]));
        }
    }
}

let instructionIndexSequence = getInstructionIndexSequence(parsedInstructions[0], 0, [0]);
