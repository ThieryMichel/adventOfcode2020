...
function hasValidProp(key, predicate, passport) {
  var value = Js_dict.get(passport, key);
  if (value !== undefined) {
    return Curry._1(predicate, Caml_option.valFromOption(value));
  } else {
    return false;
  }
}

function hasValidBirthYear(param) {
  return hasValidProp("byr", (function (byr) {
                var number = Belt_Int.fromString(byr);
                if (number !== undefined && number >= 1920) {
                  return number <= 2002;
                } else {
                  return false;
                }
...
function hasValidIssueYear(param) {
  return hasValidProp("iyr", (function (iyr) {
                var number = Belt_Int.fromString(iyr);
                if (number !== undefined && number >= 2010) {
                  return number <= 2020;
                } else {
                  return false;
                }
...
function hasValidExpirationYear(param) {
  return hasValidProp("eyr", (function (eyr) {
                var number = Belt_Int.fromString(eyr);
                if (number !== undefined && number >= 2020) {
                  return number <= 2030;
                } else {
                  return false;
                }
...
function parseHeight(value) {
  var l = value.length;
  var unit = value.substring(l - 2 | 0);
  var value$1 = Belt_Int.fromString(value.substring(0, l - 2 | 0));
  return [
          unit,
          value$1
        ];
...
function hasValidHeight(param) {
  return hasValidProp("hgt", (function (hgt) {
                var match = parseHeight(hgt);
                var value = match[1];
                switch (match[0]) {
                  case "cm" :
...
var hclRe = new RegExp("^#([0-9]|[a-f]){6}$");

function hasValidHairColor(param) {
  return hasValidProp("hcl", (function (hcl) {
                return hclRe.test(hcl);
              }), param);
}
...
function hasValidEyeColor(param) {
  return hasValidProp("ecl", (function (ecl) {
                return validColors.includes(ecl);
              }), param);
}

var pidRe = new RegExp("^[0-9]{9}$");

function hasValidPid(param) {
  return hasValidProp("pid", (function (pid) {
                return pidRe.test(pid);
              }), param);
}
...
function isPassportValid(passport) {
  if (hasValidBirthYear(passport) && hasValidIssueYear(passport) && hasValidExpirationYear(passport) && hasValidHeight(passport) && hasValidHairColor(passport) && hasValidEyeColor(passport)) {
    return hasValidPid(passport);
  } else {
    return false;
  }
}

var parsedInput = input.split("\n\n").map(parsePassport).filter(isPassportValid).length;
...
exports.hasValidIssueYear = hasValidIssueYear;
exports.hasValidExpirationYear = hasValidExpirationYear;
exports.parseHeight = parseHeight;
exports.hasValidHeight = hasValidHeight;
exports.hclRe = hclRe;
exports.hasValidHairColor = hasValidHairColor;
exports.validColors = validColors;
exports.hasValidEyeColor = hasValidEyeColor;
exports.pidRe = pidRe;
exports.hasValidPid = hasValidPid;
exports.hasValidCid = hasValidCid;
